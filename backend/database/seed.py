...
async def seed_data():
    ...
    try:
        ...
        print("Наповнення бази (без рандому)...")
        ...
        employees_data = [
            ('1003009', "$2b$12$e80yq9kXqW3LhU9J1j.sGeZ8e6yvQhF5bY/G7xZ9uU8i3s8N8m1Ki", 'Мельник', 'Анна',
             'Олексіївна', 'Менеджер', 45000.00, date(1990, 5, 14), date(2023, 5, 20), '+380951234567', 'Київ',
             'вул. Хрещатик 15', '02100'),
            ('13092007', "$2b$12$e80yq9kXqW3LhU9J1j.sGeZ8e6yvQhF5bY/G7xZ9uU8i3s8N8m1Ki", 'Ткаченко', 'Максим',
             'Петрович', 'Менеджер', 45000.00, date(1988, 11, 22), date(2024, 2, 14), '+380957654321', 'Київ',
             'вул. Володимирська 42', '02100'),
            ('1337', "$2b$12$e80yq9kXqW3LhU9J1j.sGeZ8e6yvQhF5bY/G7xZ9uU8i3s8N8m1Ki", 'Коваленко', 'Роман',
             'Сергійович', 'Менеджер', 45000.00, date(1993, 3, 18), date(2025, 3, 7), '+380689876543', 'Київ',
             'вул. Велика Васильківська 28', '02100'),
        ...
        card_discounts = {c[0]: c[8] for c in customers_data}
        customer_card_numbers = list(card_discounts.keys())
        ...
        products_data = [
            ...
            (217, 1, "Молоко", "Простоквашино", "2.5%, 900мл"), (218, 2, "Шинка", "М'ясна гільдія", "нарізка"),
            (219, 3, "Яблука", "Україна", "Гала, 1кг"), (220, 4, "Кока-Кола", "Coca-Cola Company", "1.5л"),
            (221, 5, "Гречка", "Жменька", "800г"), (222, 6, "Шоколад чорний", "Світоч", "100г"),
            (223, 7, "Засіб для миття посуду", "Fairy", "500мл"), (224, 8, "Хліб пшеничний", "Київхліб", "Формовий"),
            (225, 9, "Морозиво", "Рудь", "пломбір, 400г"), (226, 10, "Філе хека", "Аквамарин", "Заморожене, 500г"),
            (227, 11, "Кетчуп", "Чумак", "томатний, 300г"), (228, 12, "Пюре фруктове", "Gerber", "яблуко, 125г"),
            (229, 13, "Корм для котів", "Whiskas", "курка, 400г"),
            (230, 14, "Зошит", "Школярик", "Шкільний, 48 аркушів"),
            (231, 1, "Кефір", "Яготинське", "1%, 900мл"), (232, 2, "Сардельки", "Глобино", "вищий гатунок"),
            (233, 3, "Помідори", "Україна", "Червоні, 1кг"), (234, 4, "Мінеральна вода", "Боржомі", "0.75л"),
            (235, 5, "Цукор", "Цукорок", "Пісок, 1кг"), (236, 6, "Печиво", "Roshen", "Марія, 200г"),
            (237, 7, "Пральний порошок", "Persil", "1.5кг"),
            (238, 8, "Булочка з маком", "Власний цех", "Свіжа випічка"),
            (239, 9, "Вареники з картоплею", "Легко", "1кг"), (240, 10, "Оселедець", "Рибний світ", "Солоний, 500г")
        ]
        await conn.executemany("""
        ...
        for p_id, _, _, _, _ in products_data:
            ...
            store_products_data.append((upc_reg, None, p_id, selling_price_reg, 5000, False))
            all_upcs_and_prices.append((upc_reg, selling_price_reg))
            ...
            if p_id % 3 == 0:
                ...
                store_products_data.append((upc_prom, upc_reg, p_id, selling_price_prom, 5000, True))
                all_upcs_and_prices.append((upc_prom, selling_price_prom))
                promo_upcs_and_prices.append((upc_prom, selling_price_prom))
        ...
        for i in range(1, num_checks + 1):
            check_id = str(i)
            empl_id = cashier_ids[i % len(cashier_ids)]
            p_date = base_date + timedelta(minutes=15 * i)

            check_sum = 0.0
            selected_items = []

            if i <= 10:
                card_num = customer_card_numbers[i % 4]

                for upc_code, price in promo_upcs_and_prices:
                    qty = (i % 2) + 1
                    selected_items.append((upc_code, qty, price))
                    check_sum += price * qty
            else:
                if i % 3 == 0:
                    card_num = None
                else:
                    card_num = customer_card_numbers[i % len(customer_card_numbers)]

                num_items_in_check = (i % 5) + 1
                start_idx = (i * 3) % len(all_upcs_and_prices)

                for j in range(num_items_in_check):
                    upc_code, price = all_upcs_and_prices[(start_idx + j) % len(all_upcs_and_prices)]
                    qty = (j % 3) + 1
                    selected_items.append((upc_code, qty, price))
                    check_sum += price * qty

            discount_percent = card_discounts.get(card_num, 0) if card_num else 0
            final_sum = check_sum * (1 - discount_percent / 100.0)
            vat = final_sum * 0.2

            checks_to_insert.append((check_id, empl_id, card_num, p_date, round(final_sum, 2), round(vat, 2)))

            for upc_code, qty, price in selected_items:
